...
App.pieAPI = function(){
  ...
  $.get("states/pie_data/" + stateAbrev, function(data){
    ...
    var d3Data = [];
    var pieSources = [];
    var pieHash = {type: 0, amount: 0};
    for (source in data){
      pieSources.push(source);
    }
    for (var j = 0; j < pieSources.length; j++){
      pieHash.type = pieSources[j];
      pieHash.amount = energyTotals[pieSources[j]];
      d3Data.push(pieHash);
    }
    console.log(pieHash);
    App.makePieChart(d3Data);
  });
};
...
App.makePieChart = function(dataHash){
  ...
  d3.json(dataHash, function(error, data) {
